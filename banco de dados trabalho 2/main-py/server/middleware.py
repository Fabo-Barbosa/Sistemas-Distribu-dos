...
# --- CONFIGURAÇÃO GLOBAL ---
# SEU IP (O servidor de banco de dados - Windows)
IP_DO_WINDOWS = "10.16.0.167" 
...
# Credenciais do Banco (O usuário que criamos)
DB_USER = "devmaster"
DB_PASS = "@DevMaster123"
...
class Middleware:
    ...
    def calcular_checksum(self, conteudo):
        if isinstance(conteudo, str): conteudo = conteudo.encode()
        return sum(conteudo)

    def conectar_banco(self):
        # Define qual banco usar baseado no ID
        if self.meu_id == 1:
            nome_banco = "db_no1"
        elif self.meu_id == 2:
            nome_banco = "db_no2"
        ...
        try:
            ...
            print(f"Erro Conexao Banco ({nome_banco}): {e}")
            return None

    def empacotar(self, tipo, id_origem, conteudo):
        if isinstance(conteudo, str): conteudo = conteudo.encode('utf-8')
        conteudo_bytes = conteudo[:1024].ljust(1024, b'\0')
        checksum = self.calcular_checksum(conteudo)
        return struct.pack(FORMATO_PACOTE, tipo, id_origem, conteudo_bytes, checksum)
    ...
    def iniciar_eleicao(self):
        if self.estou_em_eleicao: return
        self.estou_em_eleicao = True
        print(f"!!! Lider caiu. Iniciando Eleicao... !!!")
        
        tenho_chances = True
        for no in self.lista_nos:
            if no['id'] > self.meu_id: # Bully: só chamo quem é maior
                try:
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                        s.settimeout(1.0)
                        if s.connect_ex((no['ip'], no['porta'])) == 0:
                            print(f"-> No maior ({no['id']}) avisado.")
                            tenho_chances = False
                except: pass
    ...
    def rotina_heartbeat(self):
        while True:
            time.sleep(3)
            self.propagar_para_vizinhos(MSG_HEARTBEAT, self.meu_id, "HB")
            
            if self.id_lider_atual != self.meu_id and self.id_lider_atual != 0:
                if time.time() - self.ultima_msg_lider > 10:
                    self.iniciar_eleicao()
    ...
    def iniciar(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
            ...
            server.bind(('0.0.0.0', minha_porta))
            server.listen(5)
            print(f"--- Ouvindo na Porta {minha_porta} ---")
            
            while True:
                try:
                    conn, addr = server.accept()
                    with conn:
                        dados = conn.recv(TAMANHO_PACOTE)
                        if not dados: continue
                        tipo, id_origem, conteudo, chk = self.desempacotar(dados)
                        
                        if tipo == MSG_HEARTBEAT:
                            if id_origem == self.id_lider_atual: self.ultima_msg_lider = time.time()
                            ...
                            print(f"Comando recebido: {conteudo}")
                            db = self.conectar_banco()
                            resp = "Erro DB"
                            if db:
                                cur = db.cursor()
                                try:
                                    ...
                                    if conteudo.strip().upper().startswith("SELECT"):
                                        rows = cur.fetchall()
                                        resp = str(rows)
                                    else:
                                        db.commit() # Importante para salvar
                                        resp = "Sucesso"
                                        # Se veio do cliente (ID 0), replica!
                                        if id_origem == 0:
                                            print("Iniciando Broadcast...")
                                            threading.Thread(target=self.propagar_para_vizinhos, args=(MSG_QUERY, self.meu_id, conteudo)).start()
                                except Error as e: resp = str(e)
                                finally: db.close()
